...
from vllm.oracle_lenpred import predict_length as oracle_predict_length
...
class Processor:
    ...
    def process_inputs(
        ...
        # If the OpenAI layer provided an oracle-specific prompt
        # (e.g., user-only content without the long system prompt),
        # prefer that for oracle length prediction. This keeps the
        # online oracle aligned with the training pipeline used in
        # ATHENA/ORACLE, where prompts were constructed by joining
        # non-system message contents.
        oracle_prompt_str: Optional[str] = None
        if isinstance(prompt, dict):
            oracle_prompt_candidate = prompt.get("oracle_prompt")
            if isinstance(oracle_prompt_candidate, str) and oracle_prompt_candidate.strip():
                oracle_prompt_str = oracle_prompt_candidate
        ...
        if decoder_inputs["type"] == "embeds":
            ...
            prompt_str = decoder_inputs.get("prompt")
            prompt_token_ids = decoder_inputs["prompt_token_ids"]
            prompt_embeds = None

            # 对于仅提供 token_ids 的请求（如 ChatCompletion 路径），在需要时回退到
            # tokenizer.decode 以为 oracle 重建可读的 prompt 文本。
            if prompt_str is None and self.tokenizer is not None \
        ...
        # Oracle length prediction (optional). Only run for generative
        # requests with a concrete text prompt.
        # 优先使用上层提供的 oracle_prompt_str（通常为去掉 system 的 user-only
        # prompt），否则退回到 decoder_inputs 中的 prompt_str。
        oracle_metadata = None
        oracle_input_str = oracle_prompt_str or prompt_str
        if sampling_params is not None and oracle_input_str:
            oracle_metadata = oracle_predict_length(oracle_input_str)
