...
class OpenAIServing:
    ...
    async def _collect_batch(
        ...
        try:
            ...
            # Lightweight logging for oracle-predicted vs actual lengths.
            # We log once per request (first result in the batch).
            try:
                first_res = ctx.final_res_batch[0]
                if isinstance(first_res, RequestOutput):
                    oracle_meta = getattr(first_res, "oracle_metadata", None)
                    if oracle_meta is not None:
                        prompt_len = oracle_meta.get("prompt_length")
                        pred_len_reg = oracle_meta.get(
                            "regression_estimated_length")
                        pred_len_cls = oracle_meta.get(
                            "class_estimated_length")

                        actual_lens: list[int] = []
                        for out in first_res.outputs:
                            try:
                                actual_lens.append(len(out.token_ids))
                            except Exception:
                                continue

                        logger.info(
                            "oracle_lenpred request_id=%s model=%s "
                            "prompt_tokens=%s predicted_len_reg=%s "
                            "predicted_len_cls=%s actual_output_lengths=%s",
                            ctx.request_id,
                            ctx.model_name,
                            prompt_len,
                            pred_len_reg,
    ...
    async def _preprocess_chat(
        ...
        # For oracle length predictor integration: build a lightweight,
        # training-style prompt string that is closer to the oracle's
        # training data. In the ATHENA/ORACLE pipeline, the length
        # predictor was trained on prompts constructed by *excluding*
        # the long system prompt and concatenating user contents
        # (and optional inputs). We approximate that here by joining
        # all non-system message contents with newlines.
        oracle_prompt_parts: list[str] = []
        for m in messages:
            # messages is a list of pydantic models or dict-like
            # ChatCompletionMessageParam. We access role/content
            # defensively.
            ...
            if isinstance(content, str) and content:
                oracle_prompt_parts.append(content)
        if oracle_prompt_parts:
            # This field is consumed only by the v1 Processor for
            # oracle_lenpred; it does not affect normal generation.
            engine_prompt["oracle_prompt"] = "\n".join(oracle_prompt_parts)
